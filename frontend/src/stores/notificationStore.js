...
const useNotificationStore = create((set, get) => ({
  ...
  /**
   * Pushes an in-app notification directly into the notification list without
   * making any API call. Used by other stores (e.g. aiStore) to surface
   * events like report generation inside the bell dropdown.
   */
  pushLocal: (notification) => {
    ...
    const notificationSettings = get().settings || defaultNotificationSettings;
    if (notificationSettings.notifications_enabled) browserNotify(item.title, item.body);
    if (get().voiceEnabled && notificationSettings.voice_notifications_enabled) {
      speakText(`${item.title}. ${item.body}`);
    }
  },
